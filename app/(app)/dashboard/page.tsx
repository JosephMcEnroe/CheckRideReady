...
import { Clock, PlayCircle, TrendingUp, Trophy } from "lucide-react";
      ...
      <div className="bg-gradient-to-r from-[#1e3a5f] to-[#2d5a8f] rounded-xl p-8 text-white shadow-lg">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-white/80">Practice with our AI Designated Pilot Examiner and get instant feedback</p>
          </div>
          <Link
            href="/sessions/new"
        ...
        <div className="bg-card rounded-xl border border-border p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-foreground">{avgScore === null ? "-" : `${avgScore}%`}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-foreground">{passRate === null ? "-" : `${passRate}%`}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card rounded-xl border border-border p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            {latestSession?.last_result ? <StatusBadge status={latestSession.last_result} /> : <StatusBadge status="IN_PROGRESS" />}
          </div>
          ...
          {!loading && !error && latestSession && (
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium text-foreground">{formatDateTime(latestSession.created_at)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Topic</span>
                <span className="text-sm font-medium text-foreground">{latestSession.topic || "Untitled"}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Score</span>
                <span className="text-sm font-medium text-foreground">{latestSession.score === null ? "-" : `${latestSession.score}%`}</span>
              </div>
            </div>
          )}
            ...
            {sessions.slice(0, 3).map((session) => (
              <div key={session.id} className="flex items-center justify-between p-3 bg-secondary/50 rounded-lg">
                <div className="space-y-1">
                  <p className="text-sm font-medium text-foreground">{session.topic || "Untitled Session"}</p>
                  <p className="text-xs text-muted-foreground">{formatDateTime(session.created_at)}</p>
                </div>
                {session.last_result ? <StatusBadge status={session.last_result} /> : <StatusBadge status="IN_PROGRESS" />}
              </div>
            ))}
          </div>
          <Link href="/sessions" className="block w-full text-center text-[#1e3a5f] hover:text-[#2d5a8f] font-medium mt-4 transition-colors">
            View All Sessions
          </Link>
        </div>
      </div>
